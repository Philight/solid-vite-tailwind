$(document).ready(function () {
   ...
   * LOAD SETTINGS
   *
   * settings for number of days selection, pages
   */
   ...
   * RENDER DAYS
   *
   * render available Days from settings, use template
   */
   ...
   * DAY SELECTION
   *
   * Trigger "renderTimes", input[time](onChange)
   */
  ...
  /**
   * CALL API CashAPI.getTimedOrderTimes()
   *
   * get a list of available times & capacity, render orders' TimesView
   * @params selectedDate (day)
   */
  ...
  $(timedOrderTimesContainer, timedOrderWrapper).on(
    ...
    function () {
      ...
      /**
       * IF time = "selected" & "active", clicking again unselect
       */
      if (active) {
        $(timedOrderTimesContainer)
          .children()
  ...
  */
  ...
  StoreManager.registerReadyCallback(function () {
    ...
    */
     ...
     * Initial rendering
     *
     * Trigger "renderDays", renderTimes", input[time](onChange)
     */
